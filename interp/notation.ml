...
module InnerPrimToken = struct
  ...
  let ofNumeral n s =
    if s then Z.of_string n else Z.neg (Z.of_string n)
  ...
  let mkNumeral n =
    if Z.(leq zero n) then Numeral (Z.to_string n, true)
    else Numeral (Z.to_string (Z.neg n), false)
...
let big2raw n =
  if Z.(leq zero n) then (Z.to_string n, true)
  else (Z.to_string (Z.neg n), false)
