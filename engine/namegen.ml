...
let head_name sigma c = (* Find the head constant of a constr if any *)
  let rec hdrec c =
    ...
    | Proj (kn,_) ->
      let kn = Projection.make (Nametab.get_compat_projection_for_projector kn) true in
      Some (Label.to_id (Constant.label (Projection.constant kn)))
...
let hdchar env sigma c =
  let rec hdrec k c =
    ...
    | Proj (kn,_) ->
      let kn = Projection.make (Nametab.get_compat_projection_for_projector kn) true in
      lowercase_first_char (Label.to_id (Constant.label (Projection.constant kn)))
