...
VERNAC COMMAND FUNCTIONAL EXTEND HintResolveIffLR CLASSIFIED AS SIDEFF
  ...
  [ fun ~atts ~st -> begin
        ...
        add_hints_iff ?locality:atts.local true lc n bl;
        st
      end
  ]
| [ "Hint" "Resolve" "->" ne_global_list(lc) natural_opt(n) ] ->
  [ fun ~atts ~st -> begin
        let open Vernacexpr in
        add_hints_iff ?locality:atts.local true lc n ["core"];
        st
      end
  ]
END
...
VERNAC COMMAND FUNCTIONAL EXTEND HintResolveIffRL CLASSIFIED AS SIDEFF
  ...
  [ fun ~atts ~st -> begin
        let open Vernacexpr in
        add_hints_iff ?locality:atts.local false lc n bl;
        st
      end
  ]
| [ "Hint" "Resolve" "<-" ne_global_list(lc) natural_opt(n) ] ->
  [ fun ~atts ~st -> begin
        let open Vernacexpr in
        add_hints_iff ?locality:atts.local false lc n ["core"];
        st
      end
  ]
END
