...
let round_num q = if Q.(leq zero) q then ceilingQ q else floorQ q
...
let decimalize =
  let rec normalize y =
    if Q.lt (Q.abs y) Q.(inv (of_int 10)) then normalize Q.(mul (of_int 10) y) - 1
    else if Q.geq (Q.abs y) Q.one then normalize Q.(div y (of_int 10)) + 1
    else 0 in
  fun d x ->
    if Q.(equal zero) x then "0.0" else
    let y = Q.abs x in
    let e = normalize y in
    let z = Q.add (Q.mul (Q.of_bigint (pow10 (-e))) y) Q.one in
    let k = round_num (Q.mul (Q.of_bigint (pow10 d)) z) in
    (if Q.(gt zero) x then "-0." else "0.") ^
...
let (|-->) x y a = if Q.(equal zero) y then a else (x |-> y) a
...
let matrix_cmul c (m:matrix) =
  ...
  if Q.(equal zero) c then matrix_0 (i,j)
  else (i,j),mapf (Q.mul c) (snd m);;
...
let matrix_add (m1:matrix) (m2:matrix) =
  ...
  else (d1,combine Q.add Q.(equal zero) (snd m1) (snd m2) :matrix)
...
let poly_const c =
  if Q.(equal zero) c then poly_0 else (monomial_1 |=> c)

let poly_cmul c (p:poly) =
  if Q.(equal zero) c then poly_0
  else mapf (Q.mul c) p

let poly_neg (p: poly) = (mapf Q.neg p :poly)

let poly_add (p1:poly) (p2:poly) =
  (combine Q.add Q.(equal zero) p1 p2 :poly)

let poly_sub p1 p2 = poly_add p1 (poly_neg p2);;

let poly_cmmul (c,m) (p:poly) =
  if Q.(equal zero) c then poly_0
  else if m = monomial_1 then mapf (Q.mul c) p
  else foldl (fun a m' d -> (monomial_mul m m' |-> Q.mul c d) a) poly_0 p
...
let string_of_cmonomial (c,m) =
  ...
  else if Q.(equal one) c then string_of_monomial m
...
let string_of_poly (p:poly) =
  ...
  let s =
    List.fold_left (fun a (m,c) ->
             if Q.lt c Q.zero then a ^ " - " ^ string_of_cmonomial(Q.neg c, m)
...
let decimal =
  ...
  let decimalfrac = atleast 1 numeral
    >> (fun s -> Q.div (Q.of_string (implode s)) (Q.of_bigint (pow10 (List.length s)))) in
  let decimalsig =
    decimalint ++ possibly (a "." ++ decimalfrac >> snd)
    >> (function (h,[x]) -> Q.mul h x | (h,_) -> h) in
  let signed prs =
       a "-" ++ prs >> ((o) Q.neg snd)
    || a "+" ++ prs >> snd
    || prs in
  let exponent = (a "e" || a "E") ++ signed decimalint >> snd in
    ...
    >> (function (h,[x]) -> Q.mul h (Q.of_bigint (pow10 (Q.to_int x))) | (h,_) -> h)
...
let scale_then =
  ...
  and maximal_element amat acc =
    foldl (fun maxa m c -> Q.max maxa (Q.abs c)) acc amat in
  fun solver obj mats ->
    let cd1 = List.fold_right common_denominator mats Z.one |> Q.of_bigint in
    let cd2 = common_denominator (snd obj)  Z.one |> Q.of_bigint in
    let mats' = List.map (mapf (Q.mul cd1)) mats
    ...
    let scal1 = pow2 (20-int_of_float(log(Q.to_float max1) /. log 2.0)) in
    let scal2 = pow2 (20-int_of_float(log(Q.to_float max2) /. log 2.0)) in
    let mats'' = List.map (mapf (fun x -> Q.mul x scal1)) mats'
...
let nice_rational n x = Q.div (round_num (Q.mul n x)) n
...
let in_convex_hull pts pt =
  ...
  let mat =
    (m,n),
    itern 1 pts2 (fun pts j -> itern 1 pts (fun x i -> (i,j) |-> Q.of_int x))
                 (iter (1,n) (fun i -> (v + i,i+1) |-> Q.one) undefined) in
...
let equation_cmul c eq =
  if Q.(equal zero) c then Empty else mapf (Q.mul c) eq

let equation_add eq1 eq2 = combine Q.add Q.(equal zero) eq1 eq2

let equation_eval assig eq =
  let value v = apply assig v in
  foldl (fun a v c -> Q.add a (Q.mul (value v) c)) Q.zero eq
...
let eliminate_all_equations one =
  ...
  let rec eliminate dun eqs =
    ...
    | eq::oeqs ->
        ...
        let a = apply eq v in
        let eq' = equation_cmul Q.(div minus_one a) (undefine v eq) in
        let elim e =
          let b = tryapplyd e v Q.zero in
          if Q.(equal zero) b then e else
          equation_add e (equation_cmul (Q.(div (neg b)) a) eq) in
...
let diag m =
  ...
  let rec diagonalize i m =
    ...
    if Q.lt a11 Q.zero then failwith "diagonalize: not PSD"
    else if Q.(equal zero) a11 then
      if is_zero(row i m) then diagonalize (i + 1) m
      else failwith "diagonalize: not PSD"
    else
      let v = row i m in
      let v' = mapa (fun a1k -> Q.div a1k a11) v in
      let m' =
      (n,n),
      iter (i+1,n) (fun j ->
          iter (i+1,n) (fun k ->
              ((j,k) |--> (Q.sub (element m (j, k)) (Q.mul (element v j) (element v' k))))))
          undefined in
...
let rec enumerate_products d pols =
  if d = 0 then [poly_const Q.one,Rational_lt Q.one] else if d < 0 then [] else
  match pols with
...
let bmatrix_add = combine Q.add Q.(equal zero)

let bmatrix_cmul c bm =
  if Q.(equal zero) c then undefined
  else mapf (Q.mul c) bm
...
let real_positivnullstellensatz_general linf d eqs leqs pol =
  ...
  let mk_idmultiplier k p =
    ...
    mons,
    List.fold_right (fun (m,n) -> (m |-> ((-k,-n,n) |=> Q.one))) nons undefined in
  let mk_sqmultiplier k (p,c) =
    ...
    List.fold_right (fun (m1,n1) ->
      List.fold_right (fun (m2,n2) a ->
          ...
          let c = if n1 = n2 then Q.one else Q.of_int 2 in
  ...
  and obj = List.length pvs,
            itern 1 pvs (fun v i -> (i |--> tryapplyd diagents v Q.zero))
                        undefined in
  ...
  let vec,ratdias =
    ...
    else tryfind find_rounding (List.map Q.of_int (1--31) @
                                List.map pow2 (5--66)) in
  let newassigs =
    List.fold_right (fun k -> List.nth pvs (k - 1) |-> element vec k)
           (1--dim vec) ((0,0,0) |=> Q.minus_one) in
  let finalassigs =
...
let term_of_monomial =
  fun m -> if m = monomial_1 then Const Q.one else
...
let term_of_cmonomial =
  fun (m,c) ->
    ...
    else if Q.(equal one) c then term_of_monomial m
...
let sumofsquares_general_symmetry tool pol =
  ...
  let sym_eqs =
    ...
    let mk_eq cls acc =
      match cls with
        [] -> raise Sanity
      | [h] -> acc
      | h::t -> List.map (fun k -> (k |-> Q.minus_one) (h |=> Q.one)) t @ acc in
  ...
  let mk_matrix v =
   ((n,n),
    foldl (fun m (i,j) ass -> let c = tryapplyd ass v Q.zero in
                              if Q.(equal zero) c then m else
                              ((j,i) |-> c) (((i,j) |-> c) m))
  ...
  let rat,dia =
    if pvs = [] then
       ...
       tryfind find_rounding (List.map Q.of_int (1--31) @
                              List.map pow2 (5--66)) in
  let poly_of_lin(d,v) =
