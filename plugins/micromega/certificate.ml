...
let simplify f sys =
  let (sys',b) =
  List.fold_left (fun (sys',b) c -> match f c with
                                | None    -> (c::sys',b)
                                | Some c' -> (c'::sys',true)
    ) ([],false) sys in
...
let elim_linear_equality strict (sys:WithProof.t list)  =
  let (oeq,sys) =   extract (is_substitution strict) sys in
  match oeq with
  | None -> None (* Nothing to do *)
  | Some(v,pc) -> simplify (WithProof.linear_pivot sys pc v) sys
...
(* Too aggressive - should be limited to linear case

let elim_every_substitution  sys =
  iterate_until_stable (elim_linear_equality true) sys
 *)

let elim_every_linear_equality sys =
    iterate_until_stable (elim_linear_equality false) sys

let saturate_linear_equality_non_linear sys =

  let rec elim sys acc =
    let (oeq,sys) = extract (is_substitution false) sys in
    match oeq with
    | None -> acc (* We are done *)
    | Some(v,pc) ->
       let nc = saturate (non_linear_pivot sys pc v) sys in
       elim sys (nc@acc) in
...
let nlinear_prover prfdepth sys =
  ...
  let sys = elim_every_linear_equality sys in
