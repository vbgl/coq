...
type reloc_info =
  ...
  | Reloc_proj_name of Projector.t
...
let hash_reloc_info r =
  ...
  | Reloc_proj_name p -> combinesmall 4 (Projector.hash p)
...
(*      out_int env (Array.length tbl_const + (Array.length tbl_block lsl 23)); *)
  ...
  | Kproj (n, _ind as p) -> out env opPROJ; out_int env n; slot_for_proj_name env p
